...
struct obj *no_wish;
{
    ...
    very = rechrg = blessed = uncursed = iscursed = ispoisoned =
        ...
        locked = unlocked = broken = isdrained = halfdrained = real = fake = 0;
    tvariety = RANDOM_TIN;
    mntmp = NON_PM;
#define UNDEFINED 0
#define EMPTY 1
#define SPINACH 2
    ...
    for (;;) {
        ...
        if (!strncmpi(bp, "an ", l = 3) || !strncmpi(bp, "a ", l = 2)) {
            ...
            ispoisoned = POT_SICKNESS;
        } else if (!strncmpi(bp, "drugged ", l = 8)) {
            ispoisoned = POT_SLEEPING;
            ...
            ispoisoned = POT_FILTH;
        } else if (!strncmpi(bp, "potion-coated ", l = 14)) {
            ispoisoned = POT_GAIN_ABILITY + rn2(POT_VAMPIRE_BLOOD - POT_GAIN_ABILITY);
