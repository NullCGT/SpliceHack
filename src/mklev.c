...
static void FDECL(mktree, (struct mkroom *));
static void FDECL(mkfurnace, (struct mkroom *));
        ...
        if (levl[x][y].doormask & D_TRAPPED) {
            ...
            if (level_difficulty() >= 9 && !rn2(5)
                && !((g.mvitals[PM_SMALL_MIMIC].mvflags & G_GONE)
                     ...
                     && (g.mvitals[PM_KILLER_MIMIC].mvflags & G_GONE))) {
...
boolean lava;
{
    ...
    while (count++ < 2000) {
      	...
      	chance = 0;
      	if (rnum >= 0 && g.rooms[rnum].rtype != OROOM) chance = 0;
...

    {
        ...
        if (wizard && nh_getenv("SHOPTYPE"))
            ...
            mkroom(ZOO);
                 ...
                 && !(g.mvitals[PM_RUST_MONSTER].mvflags & G_GONE)) 
            mkroom(ARMORY);
            ...
            mkroom(BEEHIVE);
                 ...
                 && !(g.mvitals[PM_AMALGAMATION].mvflags & G_GONE))
            mkroom(LAB);
