        ...
        for (;;) {
            for (n = 0, i = 0; roles[i].name.m; i++) {
                if (ok_role(i, flags.initrace, flags.initgend, flags.initalign)) {
                    if (flags.initgend >= 0 && flags.gender == GEND_F && roles[i].name.f)
                        choices[n] = roles[i].name.f;
                    else if (flags.initgend >= 0 && flags.gender == GEND_N && roles[i].name.n)
...
curses_display_splash_window()
{
    ...
    int x_start;
    int y_start;
    int which_variant = SPLICEHACK_CURSES; /* Default to NetHack */
