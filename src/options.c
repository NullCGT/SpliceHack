...
static struct Comp_Opt {
    ...
    { "orientation", "your sexual orientation or the gender you are attracted to.", 8, DISP_IN_GAME },
...
char *buf;
{
    ...
    if (!strcmp(optname, "align_message")
        ...
        Sprintf(buf, "%s", g.birdname[0] ? g.birdname : none);
#ifdef BACKWARD_COMPAT
        ...
        Sprintf(buf, "%s", g.dragonname[0] ? g.dragonname : none);
    else if (!strcmp(optname, "ratname"))
		Sprintf(buf, "%s", g.ratname[0] ? g.ratname : none );
