...
#define PCHAR2(idx, ch, sym, tilenm, desc, clr) \
    ...
    PCHAR( 19, '#',  S_dead_tree, "dead tree", CLR(CLR_BROWN))
    PCHAR( 20, '.',  S_room, "floor of a room", CLR(CLR_GRAY))
    PCHAR( 21, '.',  S_darkroom, "dark part of a room", CLR(CLR_BLACK))
    PCHAR2(22, '#',  S_corr, "dark corridor", "corridor", CLR(CLR_GRAY))
    PCHAR( 23, '#',  S_litcorr, "lit corridor", CLR(CLR_GRAY))
    PCHAR2(24, '<',  S_upstair, "up stairs", "staircase up", CLR(CLR_GRAY))
    PCHAR2(25, '>',  S_dnstair, "down stairs", "staircase down", CLR(CLR_GRAY))
    PCHAR2(26, '<',  S_upladder, "up ladder", "ladder up", CLR(CLR_BROWN))
    PCHAR2(27, '>',  S_dnladder, "down ladder", "ladder down", CLR(CLR_BROWN))
    PCHAR( 28, '<',  S_brupstair, "branch staircase up", CLR(CLR_YELLOW))
    PCHAR( 29, '>',  S_brdnstair, "branch staircase down", CLR(CLR_YELLOW))
    PCHAR( 30, '<',  S_brupladder, "branch ladder up", CLR(CLR_YELLOW))
    PCHAR( 31, '>',  S_brdnladder, "branch ladder down", CLR(CLR_YELLOW))
    PCHAR( 32, '_',  S_altar, "altar", CLR(CLR_GRAY))
    PCHAR( 33, '|',  S_grave, "grave", CLR(CLR_WHITE))
    PCHAR2(34, '\\', S_throne, "throne", "opulent throne", CLR(HI_GOLD))
    PCHAR( 35, '#',  S_sink, "sink", CLR(CLR_GRAY))
    PCHAR( 36, '{',  S_furnace, "furnace", CLR(CLR_ORANGE))
    PCHAR( 37, '{',  S_fountain, "fountain", CLR(CLR_BRIGHT_BLUE))
    PCHAR( 38, '#',  S_vent, "vent", CLR(CLR_BLACK))
    PCHAR2(39, '}',  S_pool, "pool", "water", CLR(CLR_BLUE))
    PCHAR( 40, '.',  S_ice, "ice", CLR(CLR_CYAN))
    PCHAR( 41, '.',  S_bridge, "rope bridge", CLR(CLR_BROWN))
    PCHAR( 42, ',',  S_grass, "grass", CLR(CLR_GREEN))
    PCHAR( 43, '}',  S_lava, "molten lava", CLR(CLR_RED))
    PCHAR2(44, '.',  S_vodbridge, "vertical open drawbridge",
                                  "lowered drawbridge", CLR(CLR_BROWN))
    PCHAR2(45, '.',  S_hodbridge, "horizontal open drawbridge",
                                  "lowered drawbridge", CLR(CLR_BROWN))
    PCHAR2(46, '#',  S_vcdbridge, "vertical closed drawbridge",
                                  "raised drawbridge", CLR(CLR_BROWN))
    PCHAR2(47, '#',  S_hcdbridge, "horizontal closed drawbridge",
                                  "raised drawbridge", CLR(CLR_BROWN))
    PCHAR( 48, ' ',  S_air, "air", CLR(CLR_CYAN))
    PCHAR( 49, '#',  S_cloud, "cloud", CLR(CLR_GRAY))
    PCHAR( 50, '}',  S_water, "water", CLR(CLR_BLUE))
    /* end dungeon characters, begin traps */
    PCHAR( 51, '^',  S_arrow_trap, "arrow trap", CLR(HI_METAL))
    PCHAR( 52, '^',  S_dart_trap, "dart trap", CLR(HI_METAL))
    PCHAR( 53, '^',  S_falling_rock_trap, "falling rock trap", CLR(CLR_GRAY))
    PCHAR( 54, '^',  S_squeaky_board, "squeaky board", CLR(CLR_BROWN))
    PCHAR( 55, '^',  S_bear_trap, "bear trap", CLR(HI_METAL))
    PCHAR( 56, '^',  S_land_mine, "land mine", CLR(CLR_RED))
    PCHAR( 57, '^',  S_rolling_boulder_trap, "rolling boulder trap", CLR(CLR_GRAY))
    PCHAR( 58, '^',  S_sleeping_gas_trap, "sleeping gas trap", CLR(HI_ZAP))
    PCHAR( 59, '^',  S_rust_trap, "rust trap", CLR(CLR_BLUE))
    PCHAR( 60, '^',  S_fire_trap, "fire trap", CLR(CLR_ORANGE))
    PCHAR( 61, '^',  S_buzzsaw_trap, "buzzsaw trap", CLR(CLR_CYAN))
    PCHAR( 62, '^',  S_ice_block_trap, "ice block trap", CLR(CLR_BRIGHT_BLUE))
    PCHAR( 63, '^',  S_whirlwind_trap, "whirlwind trap", CLR(CLR_WHITE))
    PCHAR( 64, '^',  S_pit, "pit", CLR(CLR_BLACK))
    PCHAR( 65, '^',  S_spiked_pit, "spiked pit", CLR(CLR_BLACK))
    PCHAR( 66, '^',  S_hole, "hole", CLR(CLR_BROWN))
    PCHAR( 67, '^',  S_trap_door, "trap door", CLR(CLR_BROWN))
    PCHAR( 68, '^',  S_teleportation_trap, "teleportation trap", CLR(CLR_MAGENTA))
    PCHAR( 69, '^',  S_level_teleporter, "level teleporter", CLR(CLR_MAGENTA))
    PCHAR( 70, '^',  S_magic_portal, "magic portal", CLR(CLR_BRIGHT_MAGENTA))
    PCHAR( 71, '"',  S_web, "web", CLR(CLR_GRAY))
    PCHAR( 72, '^',  S_statue_trap, "statue trap", CLR(CLR_GRAY))
    PCHAR( 73, '^',  S_magic_trap, "magic trap", CLR(HI_ZAP))
    PCHAR2(74, '^',  S_anti_magic_trap, "anti magic trap", "anti-magic field", CLR(HI_ZAP))
    PCHAR( 75, '^',  S_polymorph_trap, "polymorph trap", CLR(CLR_BRIGHT_GREEN))
    PCHAR( 76, '~',  S_vibrating_square, "vibrating square", CLR(CLR_MAGENTA))
    /* end traps, begin special effects */
    /* zap colors are changed by map_glyphinfo() to match type of beam */
    PCHAR2(77, '|',  S_vbeam, "vertical beam", "", CLR(CLR_GRAY))
    PCHAR2(78, '-',  S_hbeam, "horizontal beam", "", CLR(CLR_GRAY))
    PCHAR2(79, '\\', S_lslant, "left slant beam", "", CLR(CLR_GRAY))
    PCHAR2(80, '/',  S_rslant, "right slant beam", "", CLR(CLR_GRAY))
    PCHAR2(81, '*',  S_digbeam, "dig beam", "", CLR(CLR_WHITE))
    PCHAR2(82, '!',  S_flashbeam, "flash beam", "", CLR(CLR_WHITE))
    PCHAR2(83, ')',  S_boomleft, "boom left", "", CLR(HI_WOOD))
    PCHAR2(84, '(',  S_boomright, "boom right", "", CLR(HI_WOOD))
    /* 4 magic shield symbols */
    PCHAR2(85, '0',  S_ss1, "shield1", "", CLR(HI_ZAP))
    PCHAR2(86, '#',  S_ss2, "shield2", "", CLR(HI_ZAP))
    PCHAR2(87, '@',  S_ss3, "shield3", "", CLR(HI_ZAP))
    PCHAR2(88, '*',  S_ss4, "shield4", "", CLR(HI_ZAP))
    PCHAR( 89, '#',  S_poisoncloud, "poison cloud", CLR(CLR_BRIGHT_GREEN))
    PCHAR( 90, '?',  S_goodpos, "valid position", CLR(CLR_BRIGHT_GREEN))
    ...
    PCHAR2(91, '/',  S_sw_tl, "swallow top left", "", CLR(CLR_GREEN))      /* 1        */
    PCHAR2(92, '-',  S_sw_tc, "swallow top center", "", CLR(CLR_GREEN))    /* 2 Order: */
    PCHAR2(93, '\\', S_sw_tr, "swallow top right", "", CLR(CLR_GREEN))     /* 3        */
    PCHAR2(94, '|',  S_sw_ml, "swallow middle left", "", CLR(CLR_GREEN))   /* 4  1 2 3 */
    PCHAR2(95, '|',  S_sw_mr, "swallow middle right", "", CLR(CLR_GREEN))  /* 6  4 5 6 */
    PCHAR2(96, '\\', S_sw_bl, "swallow bottom left", "", CLR(CLR_GREEN))   /* 7  7 8 9 */
    PCHAR2(97, '-',  S_sw_bc, "swallow bottom center", "", CLR(CLR_GREEN)) /* 8        */
    PCHAR2(98, '/',  S_sw_br, "swallow bottom right", "", CLR(CLR_GREEN))  /* 9        */
    /* explosion colors are changed by map_glyphinfo() to match type of expl. */
    PCHAR2( 99, '/',  S_explode1, "explosion top left", "", CLR(CLR_ORANGE))      /*     */
    PCHAR2(100, '-',  S_explode2, "explosion top center", "", CLR(CLR_ORANGE))    /*     */
    PCHAR2(101, '\\', S_explode3, "explosion top right", "", CLR(CLR_ORANGE))     /*Ex.  */
    PCHAR2(102, '|',  S_explode4, "explosion middle left", "", CLR(CLR_ORANGE))   /*     */
    PCHAR2(103, ' ',  S_explode5, "explosion middle center", "", CLR(CLR_ORANGE)) /* /-\ */
    PCHAR2(104, '|',  S_explode6, "explosion middle right", "", CLR(CLR_ORANGE))  /* |@| */
    PCHAR2(105, '\\', S_explode7, "explosion bottom left", "", CLR(CLR_ORANGE))   /* \-/ */
    PCHAR2(106, '-',  S_explode8, "explosion bottom center", "", CLR(CLR_ORANGE)) /*     */
    PCHAR2(107, '/',  S_explode9, "explosion bottom right", "", CLR(CLR_ORANGE))  /*     */
#undef PCHAR
#undef PCHAR2
...
#define MONSYM(idx, ch, sym, desc) \
    ...
    MONSYM(26, DEF_ZOUTHERN, S_ZOUTHERN, "zouthern animal")
    ...
    MONSYM(34, DEF_GNOLL, S_GNOLL, "gnoll")
    /* large humanoid: giant, ettin, minotaur */
    MONSYM(35, DEF_GIANT, S_GIANT, "giant humanoid")
    MONSYM(36, DEF_INVISIBLE, S_invisible, "invisible monster")
    MONSYM(37, DEF_JABBERWOCK, S_JABBERWOCK, "jabberwock")
    MONSYM(38, DEF_KOP, S_KOP, "Keystone Kop")
    MONSYM(39, DEF_LICH, S_LICH, "lich")
    MONSYM(40, DEF_MUMMY, S_MUMMY, "mummy")
    MONSYM(41, DEF_NAGA, S_NAGA, "naga")
    MONSYM(42, DEF_OGRE, S_OGRE, "ogre")
    MONSYM(43, DEF_PUDDING, S_PUDDING, "pudding or ooze")
    MONSYM(44, DEF_QUANTMECH, S_QUANTMECH, "quantum mechanic")
    MONSYM(45, DEF_RUSTMONST, S_RUSTMONST, "rust monster or disenchanter")
    MONSYM(46, DEF_SNAKE, S_SNAKE, "snake")
    MONSYM(47, DEF_TROLL, S_TROLL, "troll")
    /* umber hulk */
    MONSYM(48, DEF_UMBER, S_UMBER, "umber hulk")
    MONSYM(49, DEF_VAMPIRE, S_VAMPIRE, "vampire")
    MONSYM(50, DEF_WRAITH, S_WRAITH, "wraith")
    MONSYM(51, DEF_XORN, S_XORN, "xorn")
    /* apelike creature includes owlbear, monkey */
    MONSYM(52, DEF_YETI, S_YETI, "apelike creature")
    MONSYM(53, DEF_ZOMBIE, S_ZOMBIE, "zombie")
    MONSYM(54, DEF_HUMAN, S_HUMAN, "human or elf")
    /* space symbol*/
    MONSYM(55, DEF_GHOST, S_GHOST, "ghost")
    MONSYM(56, DEF_GOLEM, S_GOLEM, "golem")
    MONSYM(57, DEF_DEMON, S_DEMON, "major demon")
    /* fish */
    MONSYM(58, DEF_EEL,  S_EEL, "sea monster")
    /* reptiles */
    MONSYM(59, DEF_LIZARD, S_LIZARD, "lizard")
    MONSYM(60, DEF_WORM_TAIL,  S_WORM_TAIL, "long worm tail")
    MONSYM(61, DEF_MIMIC_DEF,  S_MIMIC_DEF, "mimic")
...
#define OBJCLASS(idx, class, defsym, sym, name, explain) \
    ...
    OBJCLASS(14, ROCK_CLASS, ROCK_SYM, S_rock,
                    "large stones", "boulder or statue or sconce")
