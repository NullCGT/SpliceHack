...
static int FDECL(give, (struct monst *, struct obj *));
...
dogive()
{
    ...
    /* giving code */
    i = (g.invent) ? 0 : (SIZE(drop_types) - 1);
...
int retry;
{
    ...
    if (drop_everything) {
        ...
        n = Role_if(PM_PIRATE) ? query_objlist("What would ye like to drop?",
            &g.invent, USE_INVLET|INVORDER_SORT, &pick_list,
            PICK_ANY, all_categories ? allow_all : allow_category)
            : query_objlist("What would you like to drop?", &g.invent,
...
long timeout UNUSED;
{
    ...
    boolean munching = (body == g.context.victual.piece);
