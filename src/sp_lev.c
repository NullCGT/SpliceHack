...
count_features()
{
    ...
    g.level.flags.nfountains = g.level.flags.nsinks = g.level.flags.nfurnaces = 0;
    for (y = 0; y < ROWNO; y++)
        for (x = 0; x < COLNO; x++) {
...
lua_State *L;
{
    static const char *const features[] = { "fountain", "furnace", "sink", "pool", NULL };
    static const int features2i[] = { FOUNTAIN, FURNACE, SINK, POOL, STONE };
    schar x,y;
    int typ;
    int argc = lua_gettop(L);
