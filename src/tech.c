...
tamedog(mtmp, (struct obj *) 0);
		...
		case T_CLOBBER:
			...
			mtmp = m_at(u.ux + u.dx, u.uy + u.dy);
			if (!mtmp) {
				pline("There's no one there!");
				return (0);
			} else {
				int oldhp = mtmp->mhp;
				if (Hallucination) pline("It\'s clobbering time!");
				if (!attack(mtmp)) return(0);
				if (!DEADMONSTER(mtmp) && oldhp > mtmp->mhp) {
					You("clobber %s, causing %s to reel!", mon_nam(mtmp), mhim(mtmp));
					mtmp->mstun = 1;
					mtmp->mconf = 1;
					hurtmon(mtmp, techlev(tech_no));
				}
				t_timeout = rn1(1000, 500);
			}
			break;
...
blitz_g_slam()
{
	...
	if (!DEADMONSTER(mtmp)) {
		...
		if(!cansee(u.ux + u.dx, u.uy + u.dy))
		    ...
		    You("destroy %s!",
		    	mtmp->mtame
			    ? x_monnam(mtmp, ARTICLE_THE, "poor", TRUE, TRUE)
			    : mon_nam(mtmp));
		}
		xkilled(mtmp,0);
	    }
