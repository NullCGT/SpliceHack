...
/* constructed full path names don't need fqname() */
    ...
    envp = nh_getenv("HOME");
    if (!envp)
        Strcpy(tmp_config, ".splicehackrc");
    else
        Sprintf(tmp_config, "%s/%s", envp, ".splicehackrc");
 ...
 * ignore RAMDISK */
	    ...
	    (void) strncpy(g.ratname, bufp, PL_PSIZ-1);
    ...
    } else if (src == set_in_sysconf && match_varname(buf, "DUMPLOGFILE", 11)) {
#ifdef DUMPLOG
        if (sysopt.dumplogfile)
            free((genericptr_t) sysopt.dumplogfile);
        sysopt.dumplogfile = dupstr(bufp);
#endif
    } else if (src == set_in_sysconf && match_varname(buf, "DUMPHTMLFILE", 7)) {
#ifdef DUMPHTML
            ...
            if (access(buf, 4) == -1) {
                ...
                if (access(buf, 4) == -1) {
                    /* second alternate failed too, so revert to the
                       original default ("$HOME/.splicehackrc") for message */
                    copynchars(endp, default_configfile,
                               (int) (sizeof buf - 1 - strlen(buf)));
                }
            }
