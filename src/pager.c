...
static void FDECL(add_mon_info, (winid, struct permonst *));
static void FDECL(add_obj_info, (winid, short));
...
#define MONPUTSTR(str) putstr(datawin, ATR_BOLD, str)
    ...
    if (*buf && (pm == &mons[PM_AMALGAMATION] || pm == &mons[PM_BAD_CLONE])) {
        Sprintf(buf2, "Attacks: %s, may use the attacks of other monsters.", buf);
        MONPUTSTR(buf2);
    } else if (*buf) {
...
docontact(VOID_ARGS)
{
    ...
    /*XXX overflow possibilities*/
    Sprintf(buf, "open an issue at <%s>.",
            DEVTEAM_URL);
    putstr(cwin, 0, buf);
