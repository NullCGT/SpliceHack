...
static void shk_identify(const char *, struct monst *);
static void shk_uncurse(const char *, struct monst *);
static void shk_appraisal(const char *, struct monst *);
static void shk_weapon_works(const char *, struct monst *);
static void shk_armor_works(const char *, struct monst *);
static void shk_charge(const char *, struct monst *);
static boolean shk_obj_match(struct obj *, struct monst *);
/*static int shk_class_match, (long class, struct monst *shkp));*/
static boolean shk_offer_price(const char *, long, struct monst *);
static void shk_smooth_charge(int *, int, int);
static int identify_ok(struct obj *);
static int uncurse_ok(struct obj *);
static int weapon_ok(struct obj *);
static int armor_ok(struct obj *);
static int wand_ok(struct obj *);
static int tool_ok(struct obj *);
static int ring_ok(struct obj *);
static int spbook_ok(struct obj *);
/*
...
{
    struct obj *ygold = findgold(g.invent, TRUE);
...
call_kops(register struct monst* shkp, register boolean nearshop)
{
    ...
    if (Is_blackmarket(&u.uz)) {
      	nokops = ((g.mvitals[PM_SOLDIER].mvflags & G_GONE) &&
...
void
bars_around_portal(boolean removebars)
{
    int x, y, dx, dy;
    boolean sawit = FALSE;
    struct trap *trap = g.ftrap;
    ...
    if (ANGRY(shkp)) {
        ...
        if (!Deaf && !muteshk(shkp) && !eshkp->pbanned) {
            verbalize("%s, %s!  Welcome%s to %s %s!", Hello(shkp), g.plname,
                      eshkp->visitct++ ? " again" : "",
                      s_suffix(shkname(shkp)), shtypes[rt - SHOPBASE].name);
            verbalize("Feel free to peruse our wares, or pay to use our other services.");
        } else
            You("enter %s %s%s!",
...
shk_other_services(void)
{
	...
	/* Init your name */
	if (!is_human(g.youmonst.data))
		slang = "ugly";
	else
		slang = flags.female == FEMALE ? "lady" : "buddy";
	...
	*/
	...
	if (ESHK(shkp)->services & (SHK_ID_BASIC|SHK_ID_PREMIUM))
	     add_menu(tmpwin, &nul_glyphinfo, &any , 'i', 0, ATR_NONE,
	         "Identify", MENU_ITEMFLAGS_NONE);
  
  	/* All shops can uncurse */
	any.a_int = 2;
	if (ESHK(shkp)->services & (SHK_UNCURSE))
	     add_menu(tmpwin, &nul_glyphinfo, &any , 'u', 0, ATR_NONE,
	...
	if ((ESHK(shkp)->services & (SHK_SPECIAL_A|SHK_SPECIAL_B|SHK_SPECIAL_C))
		...
		if (ESHK(shkp)->services & (SHK_SPECIAL_A|SHK_SPECIAL_B))
			add_menu(tmpwin, &nul_glyphinfo, &any , 'w', 0, ATR_NONE,
				"Weapon-works", MENU_ITEMFLAGS_NONE);
		else
	...
	if ((ESHK(shkp)->services & (SHK_SPECIAL_A|SHK_SPECIAL_B))
		...
		any.a_int = 5;
		add_menu(tmpwin, &nul_glyphinfo, &any , 'r', 0, ATR_NONE,
...
append_honorific(char *buf)
{
    ...
    if (maybe_polyd(is_vampire(g.youmonst.data), Race_if(PM_VAMPIRE)))
        ...
        Strcat(buf, !is_human(g.youmonst.data) ? " creature"
                                             : (flags.female == FEMALE) ? " lady"
                                             : (flags.female == MALE) ? " sir" : " mx");
}
...
static void
shk_identify(const char *slang, struct monst *shkp)
{
	...
	/* Pick object */
	if ( !(obj = getobj("have identified", identify_ok, GETOBJ_NOFLAGS))) return;

    if ((guesswork = !shk_obj_match(obj, shkp))) {
		verbalize("I don't normally handle that sort of item, but I could try...");
	}
...
shk_uncurse(const char *slang, struct monst *shkp)
{
	...
	/* Pick object */
	if ( !(obj = getobj("uncurse", uncurse_ok, GETOBJ_NOFLAGS))) return;
...
static void
shk_appraisal(const char *slang, struct monst *shkp)
{
	struct obj *obj;                /* The object picked            */
	...
	/* Pick object */
	if ( !(obj = getobj("appraise", weapon_ok, GETOBJ_NOFLAGS))) return;
...
static void
shk_weapon_works(const char *slang, struct monst *shkp)
{
    struct obj *obj;
    int charge;
    winid tmpwin;
    ...
    /* Pick weapon */
    if (ESHK(shkp)->services & (SHK_SPECIAL_A | SHK_SPECIAL_B))
        obj = getobj("improve", weapon_ok, GETOBJ_NOFLAGS);
    else
        obj = getobj("poison", weapon_ok, GETOBJ_NOFLAGS);
    if (!obj) return;
...
*/
static void
shk_armor_works(const char *slang, struct monst *shkp)
{
	struct obj *obj;
	int charge;
    /*WAC - Windowstuff*/
	...
	/* Pick armor */
	if ( !(obj = getobj("improve", armor_ok, GETOBJ_NOFLAGS))) return;
	...
	any.a_int = 1;
	if (ESHK(shkp)->services & (SHK_SPECIAL_A))
		add_menu(tmpwin, &nul_glyphinfo, &any , 'r', 0, ATR_NONE, "Rust/Fireproof", MENU_ITEMFLAGS_NONE);
	any.a_int = 2;
	if (ESHK(shkp)->services & (SHK_SPECIAL_B))
		add_menu(tmpwin, &nul_glyphinfo, &any , 'e', 0, ATR_NONE, "Enchant", MENU_ITEMFLAGS_NONE);
	...
	switch(selected[0].item.a_int) {
		case 1:
		if (flags.female == MALE && is_human(g.youmonst.data))
...
*/
static void
shk_charge(const char *slang, struct monst *shkp)
{
	...
	if (shk_class_match(WAND_CLASS, shkp) == SHK_MATCH)
		obj = getobj("charge", wand_ok, GETOBJ_NOFLAGS);
	else if (shk_class_match(TOOL_CLASS, shkp) == SHK_MATCH)
		obj = getobj("charge", tool_ok, GETOBJ_NOFLAGS);
	else if (shk_class_match(RING_CLASS, shkp) == SHK_MATCH)
		obj = getobj("charge", ring_ok, GETOBJ_NOFLAGS);
	else if (shk_class_match(SPBOOK_CLASS, shkp) == SHK_MATCH)
		obj = getobj("charge", spbook_ok, GETOBJ_NOFLAGS);
	if (!obj) return;
	...
	** with that letter.
	*/
	for(obj = 0, tobj = g.invent; tobj; tobj=tobj->nobj)
	...
	if (obj->oclass == WAND_CLASS)
	{
		...
		if (obj->otyp == WAN_WISHING)
		{
			...
			money2mon(shkp, money_cnt(g.invent));
			makeknown(obj->otyp);
			bot();
		} else {
			/*
...
*/
static boolean
shk_obj_match(struct obj *obj, struct monst *shkp)
{
...
*/
static void
shk_smooth_charge(int *pcharge, int lower, int upper)
{
	int charisma;
	int bonus;
...
static int identify_ok(struct obj *obj) {
    if (!obj)
...
static int uncurse_ok(struct obj *obj) {
    if (!obj)
        return GETOBJ_EXCLUDE;
    else if (!obj->bknown)
        return GETOBJ_DOWNPLAY;
...
static int weapon_ok(struct obj *obj) {
    if (!obj)
        return GETOBJ_EXCLUDE;
...
static int armor_ok(struct obj *obj) {
    if (!obj)
        return GETOBJ_EXCLUDE;
...
static int wand_ok(struct obj *obj) {
    if (!obj)
        return GETOBJ_EXCLUDE;
...
static int tool_ok(struct obj *obj) {
    if (!obj)
        return GETOBJ_EXCLUDE;
...
static int ring_ok(struct obj *obj) {
    if (!obj)
        return GETOBJ_EXCLUDE;
...
static int spbook_ok(struct obj *obj) {
    if (!obj)
        return GETOBJ_EXCLUDE;
