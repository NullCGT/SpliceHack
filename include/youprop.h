...
#define Breathless \
    (HMagical_breathing || EMagical_breathing || breathless(Upolyd ? g.youmonst.data : &mons[g.urace.malenum]))
...
#define Hate_material(material) \
    (hates_material(g.youmonst.data, material) \
     ...
     || ((material == IRON || material == COLD_IRON) && !Upolyd && Race_if(PM_ELF)) \
     || (material == IRON || material == COLD_IRON) && !Upolyd && Race_if(PM_DROW) \
     || (material == IRON || material == COLD_IRON) && !Upolyd && Race_if(PM_CHANGELING))
