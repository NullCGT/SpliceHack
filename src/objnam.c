...
struct _readobjnam_data {
    ...
    int isdrained, halfdrained, halfeaten, mntmp, contents;
...
static const char *FDECL(Cartomancer_rarity,(int otyp));
...
static NEARDATA const struct o_range o_ranges[] = {
    ...
    { "robe", ARMOR_CLASS, MYSTIC_ROBE, ROBE_OF_STASIS },
...
struct _readobjnam_data *d;
{
    ...
    d->very = d->rechrg = d->blessed = d->uncursed = d->iscursed
        ...
        = d->unlocked = d->broken = d->real = d->fake = d->isdrained
        = d->halfdrained = 0;
    ...
    for (;;) {
        ...
        /* locked, unlocked, broken: box/chest lock states */
            ...
            d->halfeaten = 1;
                   ...
                   || !strncmpi(d->bp, "partially drained ", l = 18)) {
			d->isdrained = 1;
			d->halfdrained = 1;
        } else if (!strncmpi(d->bp, "drained ", l=8)) {
			d->isdrained = 1;
			d->halfdrained = 0;
        } else if (!strncmpi(d->bp, "charred ", l = 8)) {
            d->eroded = 2;
            d->very = 0;
        } else if (!strncmpi(d->bp, "crispy ", l = 7)) {
            d->eroded = 3;
            d->very = 0;
...
struct obj *no_wish;
{
 ...
 typfnd:
    ...
    if (d.cnt > 0 && objects[d.typ].oc_merge
        ...
        if (d.oclass == COIN_CLASS && !wizard && d.cnt > 5000) {
            d.cnt = 5000;
        }
    ...
    if (d.mntmp >= LOW_PM) {
        ...
        case SCALE_MAIL:
            ...
            if (d.mntmp >= PM_GRAY_DRAGON && d.mntmp <= PM_VOID_DRAGON)
    ...
    if (erosion_matters(d.otmp)) {
        if (d.eroded && (is_flammable(d.otmp) || is_cookable(d.otmp) || is_rustprone(d.otmp)))
    ...
    if (d.isdrained && d.otmp->otyp == CORPSE && mons[d.otmp->corpsenm].cnutrit) {
		int amt;
		d.otmp->odrained = 1;
		amt = mons[d.otmp->corpsenm].cnutrit - drainlevel(d.otmp);
		if (d.halfdrained) {
		    amt /= 2;
		    if (amt == 0)
			amt++;
