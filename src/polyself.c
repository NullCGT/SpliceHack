...
#define PROPSET(PropIndx, ON)                          \
    ...
    PROPSET(FLYING, (is_flyer(Upolyd ? mdat : &mons[g.urace.malenum]) && !is_floater(Upolyd ? mdat : &mons[g.urace.malenum])));
    PROPSET(REGENERATION, regenerates(Upolyd ? mdat : &mons[g.urace.malenum]));
#undef PROPSET
...
change_sex()
{
    ...
    else if ((already_polyd ? u.ugender : flags.gender == GEND_N) && g.urole.name.n)
...
newman()
{
 ...
 dead:      /* we come directly here if experience level went to 0 or less */
    ...
    polyman("feel like a new %s!",
            ...
            ((Upolyd ? u.ugender : flags.gender) && g.urace.individual.f)
                ? g.urace.individual.f
                : ((Upolyd ? u.ugender : flags.gender) && g.urace.individual.n)
...
int mntmp;
{
    ...
    if (flags.verbose) {
        ...
        if (attacktype(g.youmonst.data, AT_MAGC))
         		pline(use_thec, monsterc,"cast monster spells");
        if (lays_eggs(g.youmonst.data) && flags.gender == GEND_F &&
...
poly_gender()
{
     ...
     * 0/1=same meaning as flags.gender, 2=none.
     */
...
ugenocided()
{
    return (boolean) ((g.mvitals[g.urole.malenum].mvflags & G_GENOD)
                      ...
                      || (g.urace.nbnum != NON_PM
                          && (g.mvitals[g.urace.nbnum].mvflags & G_GENOD)));
}
