...
int final; /* ENL_GAMEINPROGRESS:0, ENL_GAMEOVERALIVE, ENL_GAMEOVERDEAD */
{
    ...
    *tmpbuf = highc(*tmpbuf); /* same adjustment as bottom line */
    ...
    Sprintf(buf, "%s the %s's attributes:", tmpbuf,
            ((Upolyd ? u.ugender : flags.gender) && g.urole.name.f)
                ? g.urole.name.f
                : g.urole.name.m);
...
int final;
{
    ...
    innategend = (Upolyd ? u.ugender : flags.gender) ? 1 : 0;
    ...
    if (Upolyd) {
        ...
        if (!is_male(uasmon) && !is_female(uasmon) && !is_neuter(uasmon))
            Sprintf(tmpbuf, "%s ", genders[flags.gender ? 1 : 0].adj);
        if (altphrasing)
...
int final;
{
    ...
    if (lays_eggs(g.youmonst.data) && flags.gender) /* Upolyd */
        you_can("lay eggs", "");
    if (u.ulycn >= LOW_PM) {
...
int final; /* used "behind the curtain" by enl_foo() macros */
{
    ...
    for (i = 0; i < acnt; ++i) {
        ...
        case ACH_NUDE:
            ...
            break;
        case ACH_NBIN:
            you_have_X("elected to play as nonbinary");
            break;
        case ACH_MINE:
        ...
        case ACH_RNK5: case ACH_RNK6: case ACH_RNK7: case ACH_RNK8:
            Sprintf(buf, "attained the rank of %s",
                    rank_of(rank_to_xlev(absidx - (ACH_RNK1 - 1)),
                            Role_switch, ACH_NBIN ? GEND_N : (achidx < 0) ? GEND_F : GEND_M));
            you_have_X(buf);
            break;
