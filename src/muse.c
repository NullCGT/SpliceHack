...
#define nomore(x)       if (g.m.has_offense == x) continue;
    ...
    for (obj = mtmp->minvent; obj; obj = obj->nobj) {
        ...
        if (obj->otyp == MAGIC_FLUTE && !u.usleep && !rn2(5)
            && obj->spe > 0 && g.multi >= 0
        ...
        nomore(MUSE_DRUM);
        if ((obj->otyp == DRUM_OF_EARTHQUAKE || obj->otyp == LEATHER_DRUM)
            && !rn2(10)
            && obj->spe > 0 && g.multi >= 0) {
            g.m.offensive = obj;
            g.m.has_offense = MUSE_DRUM;
        }
    }
...
use_offensive(struct monst* mtmp)
{
    ...
    case MUSE_MGC_FLUTE:
        ...
        if (!Deaf && distu(mtmp->mx, mtmp->my) < 5) {
            if (Sleep_resistance)
                ...
                fall_asleep(-resist_reduce(rn1(10, 10), SLEEP_RES), TRUE);
            }
        }
        g.m_using = FALSE;
        return 2;
    case MUSE_DRUM:
        if (oseen) {
            pline("%s beats on a drum!", Monnam(mtmp));
            makeknown(otmp->otyp);
        }
        g.m_using = TRUE;
        if (otmp->otyp == DRUM_OF_EARTHQUAKE) {
            do_earthquake((mtmp->m_lev - 1) / 3 + 2, mtmp->mx, mtmp->my);
...
searches_for_item(struct monst* mon, struct obj* obj)
{
    ...
    case SCROLL_CLASS:
        if (typ == SCR_TELEPORTATION || typ == SCR_CREATE_MONSTER
            || typ == SCR_EARTH || typ == SCR_FIRE || typ == SCR_ELEMENTALISM
            || typ == SCR_FIRE)
            return TRUE;
        break;
    case AMULET_CLASS:
