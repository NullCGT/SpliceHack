...
static boolean FDECL(can_practice, (int)); /* WAC for Practicing */
...
static NEARDATA const int rwep[] = {
    ...
    YA, ELVEN_ARROW, DARK_ELVEN_ARROW, ARROW, ORCISH_ARROW,
	    ...
	    || !objects[pwep[i]].oc_bimanual) && !mon_hates_material(mtmp, otmp->material))
    {
        for (i = 0; i < SIZE(pwep); i++)
        {
            if ( wep &&
	         wep->otyp == pwep[i] &&
               !(otmp->otyp == pwep[i] &&
	         dmgval(otmp, &g.youmonst) > dmgval(wep, &g.youmonst)))
	        return FALSE;
...
register struct monst *mtmp;
{
    ...
    struct obj *tmpprop = (struct obj *) &cg.zeroobj;
    ...
    /* failure */
    if (tmpprop) g.propellor = tmpprop;
    return (struct obj *) 0;
}
...
static const NEARDATA short hwep[] = {
    ...
    SPIKED_CHAIN,
    TSURUGI, RUNESWORD, ORNATE_MACE, FLAMING_LASH, 
    DWARVISH_MATTOCK, TWO_HANDED_SWORD,
...
static const struct skill_range {
    ...
    { P_FIRST_MUNDANE, P_LAST_MUNDANE, "Mundane Skills" },
};
    ...
    do {
        ...
        for (pass = 0; pass < SIZE(skill_ranges); pass++)
            for (i = skill_ranges[pass].first; i <= skill_ranges[pass].last;
                ...
                (void) skill_level_name(P_MAX_SKILL(i), sklmaxnambuf);
                ...
                if (wizard) {
                    ...
                    if (!iflags.menu_tab_sep)
                        Sprintf(buf, " %s %-*s [%s]%s", prefix, longest,
                                P_NAME(i), sklnambuf, maxed ? " MAX" : !percent ? " " : percentbuf);
                    else
                        Sprintf(buf, " %s%s\t[%s\t/ %s]\t%s", prefix, P_NAME(i),
                                    sklnambuf,
                                    sklmaxnambuf,
                                    maxed ? "   MAX" :
                                    !percent ? "      " : percentbuf);
