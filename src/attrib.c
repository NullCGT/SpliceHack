...
static const struct innate {
  ...
  con_abil[] = { { 1, &(HSick_resistance), "", "" },
                 ...
                 { 0, 0, 0, 0 } },
  dan_abil[] = { { 1, &(HStealth), "", "" },
                 { 3, &(HFast), "fleet of foot", "lead-footed" },
                 { 10, &(HSearching), "perceptive", "distractable" },
                 { 0, 0, 0, 0 } },
  dra_abil[] = { { 7, &(HFast), "quick", "slow" },
                 { 15, &(HPoison_resistance), "healthy", "" },
                 { 16, &(HSleep_resistance), "awake", "tired" },
                 { 18, &(HFire_resistance), "cool", "warmer" },
                 { 19, &(HCold_resistance), "warm", "cooler" },
                 { 21, &(HShock_resistance), "insulated", "conductive" },
                 { 23, &(HDisint_resistance), "very firm", "vulnerable" },
                 { 0, 0, 0, 0 } },
...
int pot_type;          /* type of potion used in poison */
{
    ...
    if (i == 0 && typ != A_CHA) {
          /* was previously an instant kill, now take damage AND lose attrib */
        pline_The("poison courses through you...");
        loss = d(2, 5);
