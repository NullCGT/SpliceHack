...
rank(void)
{
    return rank_of(u.role_levels[flags.initrole], Role_switch, flags.female);
}
...
const struct condmap condition_aliases[] = {
    ...
    { "all",            BL_MASK_BAREH | BL_MASK_BLIND | BL_MASK_BUSY
                        ...
                        | BL_MASK_WOUNDEDL | BL_MASK_HOLDING | BL_MASK_AFRAID },
...
const struct conditions_t conditions[] = {
    ...
    { 10, BL_MASK_AFRAID,    bl_afraid,    { "Afraid",   "Fear",  "Fr"  } },
};
...
struct condtests_t condtests[CONDITION_COUNT] = {
    ...
    { bl_afraid,    "afraid",      opt_out, TRUE, FALSE, FALSE },
};
/* condition indexing */
...
#define test_if_enabled(c) if (condtests[(c)].enabled) condtests[(c)].test
    ...
    condtests[bl_afraid].test    = (Afraid) ? TRUE : FALSE;
