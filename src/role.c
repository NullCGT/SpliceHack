...
const struct Role roles[NUM_ROLES+1] = {
    ...
    { { "Caveman", "Cavewoman", "Caveperson" },
      ...
      MH_HUMAN | MH_DWARF | MH_GNOME | MH_WERE | MH_VAMPIRE,
    ...
    { { "Dancer", 0, 0 },
      { { "Student", 0 },
        ...
        { "Lifedancer", 0, 0 },
        { "Elementalist", 0, 0 } },
      "_the Archomentals", "_the Void", "_the Elder Elemental",
      "Dan",
      "the Seasonal Festival Grounds",
      ...
      MH_HUMAN | MH_ORC | MH_ELF | MH_DEMON | MH_CHANGELING,
      ROLE_MALE | ROLE_FEMALE | ROLE_NEUTER |
          ROLE_LAWFUL | ROLE_NEUTRAL | ROLE_CHAOTIC,
      /* Str Int Wis Dex Con Cha */
      { 7, 7, 8, 10, 7, 15 },
    ...
    { { "Monk", 0, 0 },
      ...
      S_XORN,
      ART_EYES_OF_THE_OVERWORLD,
      MH_HUMAN | MH_DWARF | MH_MERFOLK | MH_GHOUL | MH_DEMON | MH_ORC | MH_CHANGELING,
      ROLE_MALE | ROLE_FEMALE | ROLE_NEUTER |
          ROLE_LAWFUL | ROLE_NEUTRAL | ROLE_CHAOTIC,
      /* Str Int Wis Dex Con Cha */
      { 10, 7, 8, 8, 7, 7 },
    ...
    { { "Valkyrie", 0, 0 },
      ...
      S_GIANT,
      ART_ORB_OF_FATE,
      MH_HUMAN | MH_DWARF | MH_ANGEL | MH_WERE | MH_MERFOLK,
 ...
 */
    ...
    if (mons[g.urace.malenum].mflags3 & M3_INFRAVISION) {
        ...
        if (g.urole.nbnum != NON_PM)
            mons[g.urole.nbnum].mflags3 |= M3_INFRAVISION;
    }
#endif /*0*/
...
boolean race;
{
    int mnum;
    if (race) {
        mnum = (flag == GEND_F && g.urace.femalenum != NON_PM)
            ? g.urace.femalenum
            : (flag == GEND_N && g.urace.nbnum != NON_PM)
                ? g.urace.nbnum
                : g.urace.malenum;
    } else {
        mnum = (flag == GEND_F && g.urole.femalenum != NON_PM)
                ? g.urole.femalenum
                : (flag == GEND_N && g.urole.nbnum != NON_PM)
                    ? g.urole.nbnum
                    : g.urole.malenum;
    }
    return mnum;
...
int flag;
{
    return (flag == GEND_F && g.urole.name.f)
                ? g.urole.name.f
