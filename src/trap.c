...
static int FDECL(trapeffect_glyph_trap, (struct monst *, struct trap *, unsigned));
static int FDECL(trapeffect_iceblock_trap, (struct monst *, struct trap *, unsigned));
static int FDECL(trapeffect_whirlwind_trap, (struct monst *, struct trap *, unsigned));
static int FDECL(trapeffect_buzzsaw_trap, (struct monst *, struct trap *, unsigned));
...
static int FDECL(disarm_whirlwind, (struct trap *));
static int FDECL(disarm_glyph, (struct trap *));
 ...
 * Return value: 0 = not immune, 1 = obviously immune, 2 = immune but in a
 * non-obvious way such as an unidentified item or hidden intrinsic providing a
 * resistance; the player should still be warned of this trap. */
boolean
...
static int
trapeffect_glyph_trap(mtmp, trap, trflags)
struct monst *mtmp;
struct trap *trap;
unsigned trflags UNUSED;
{
...
static int
trapeffect_iceblock_trap(mtmp, trap, trflags)
struct monst *mtmp;
struct trap *trap;
unsigned trflags UNUSED;
{
...
unsigned trflags;
{
    ...
    case GLYPH_OF_CHAOS:
    case GLYPH_OF_LAW:
        return trapeffect_glyph_trap(mtmp, trap, trflags);
    case ICE_BLOCK_TRAP:
        return trapeffect_iceblock_trap(mtmp, trap, trflags);
    case WHIRLWIND_TRAP:
        ...
        case 15:
            if (on_level(&u.uz, &qstart_level))
                You_feel(
                    ...
                    (flags.gender || (Upolyd && is_neuter(g.youmonst.data)))
                        ? "oddly "
                        : "");
            else
...
boolean override;
{
    ...
    if (Hallucination && !override) {
        ...
        if (nameidx == total_names) {
            boolean fem = Upolyd ? u.ugender : flags.gender;
